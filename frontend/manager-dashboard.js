...
async function loadDashboardStats() {
    try {
        const stats = await apiCall('/manager/stats', 'GET');
...
async function loadUsers() {
    try {
        const result = await apiCall('/manager/users', 'GET');
        ...
        if (result.status === 'success' && result.data.users.length > 0) {
            tbody.innerHTML = result.data.users.map(user => `
                <tr>
                    ...
                    <td>
                        <button class="btn btn-xs btn-info" onclick="viewUserDetails('${user.accountNo}')">
        ...
        document.getElementById('users-table-body').innerHTML = '<tr><td colspan="5" class="text-center text-red-500">Failed to load users</td></tr>';
    }
}
...
async function loadTransactions() {
    try {
        const result = await apiCall('/manager/transactions', 'GET');
...
async function loadLoans() {
    try {
        const result = await apiCall('/manager/loans', 'GET');
...
async function viewUserDetails(accountNo) {
    ...
    try {
        ...
        const [usersResult, transactionsResult, loansResult] = await Promise.all([
            apiCall('/manager/users', 'GET'),
            apiCall('/manager/transactions', 'GET'),
            apiCall('/manager/loans', 'GET')
        ]);
...
// Generate reports (same as admin but read-only)
    ...
    try {
        ...
        if (type === 'daily') {
            ...
            monthTransactions.forEach(t => {
                if (!typeStats[t.type]) typeStats[t.type] = { count: 0, amount: 0 };
            ...
            reportHTML = `
                ...
                <div class="overflow-x-auto max-h-96">
                    <table class="table table-sm table-zebra">
                        <thead class="sticky top-0 bg-base-200">
                            <tr><th>Account</th><th>Name</th><th>Phone</th><th>Balance</th></tr>
                        </thead>
                        <tbody>
                            ${users.map(u => `
                                <tr>
            ...
            reportHTML = `
                ...
                <div class="overflow-x-auto max-h-96">
                    <table class="table table-sm table-zebra">
                        <thead class="sticky top-0 bg-base-200">
                            <tr><th>Loan ID</th><th>Account</th><th>Type</th><th>Amount</th><th>Remaining</th></tr>
                        </thead>
                        <tbody>
                            ${loans.map(l => `
                                <tr>
            ...
            reportData = `Loans Report - ${todayStr}\n\nActive: ${loans.length}\nTotal Amount: ৳${totalLoanAmount.toFixed(2)}\nRemaining: ৳${totalRemaining.toFixed(2)}\n\nLoan ID\tAccount\tType\tAmount\tRemaining\n`;
            loans.forEach(l => {
                reportData += `${l.loanId}\t${l.accountNo}\t${l.type}\t৳${parseFloat(l.amount).toFixed(2)}\t৳${parseFloat(l.remaining).toFixed(2)}\n`;
            });
        }
        ...
        reportContent.innerHTML = '<div class="alert alert-error"><span>Failed to generate report</span></div>';
    }
}
